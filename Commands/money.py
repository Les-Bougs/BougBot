...
class MoneyCog(commands.Cog):
    ...
    async def money(self, ctx):
        ...
        sorted_dict = sorted(
            self.bot.dict_boug.values(), key=lambda x: x.money, reverse=True
        )
        ...
        for boug in sorted_dict:
            if boug.money > 0:
                embed.add_field(
                    name="\u200b",
                    value=f":bank: {boug.name}: {boug.money} :coin:",
                    inline=False,
                )
    ...
    async def bottom(self, ctx, nth: int = 5):
        ...
        sorted_dict = sorted(
            self.bot.dict_boug.values(), key=lambda x: x.money, reverse=False
        )
