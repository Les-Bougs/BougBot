...
class VoclistCog(commands.Cog):
    ...
    async def voclist(self, ctx):
        ...
        if voc_members!=[]:
            ...
            embed = discord.Embed(
                title=f":speaking_head: Membres en vocal :speaking_head:", description="\n", colour=discord.Colour.blue())
            for member in member_list:
                embed.add_field(name="\u200b", value=f"<@{member.id}>", inline=False)
            await ctx.respond(embed=embed)
        else:
            embed = discord.Embed(
                title=f"Personne en vocal :cry:", description="\n", colour=discord.Colour.blue())
            await ctx.respond(embed=embed)
    ...
    async def volontaire(self, ctx):
        ...
        if voc_members!=[]:
            ...
            embed = discord.Embed(
                title=f"<:dogekek:751132464407248986>  {volontaire} est volontaire  <:dogekek:751132464407248986>", description="\n", colour=discord.Colour.blue())
            await ctx.respond(embed=embed)
        else:
            embed = discord.Embed(
                title=f"Personne en vocal :cry:", description="\n", colour=discord.Colour.blue())
